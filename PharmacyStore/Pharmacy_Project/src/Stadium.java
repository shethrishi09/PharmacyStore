...
class Match {
    ...
    public Match(int match_id, String match, String match_date, String match_time) {
...
class LoginedUSer {
    ...
    ArrayList<Integer> getMatch(Connection con) throws Exception {
        ...
        while (rs.next()) {
            al.add(rs.getInt(1));
            Match m = new Match(rs.getInt(1), rs.getString(2), rs.getString(5), rs.getString(6));
            ll.add(m);
        }
        ll.display();
        return al;
            ...
            switch (choice) {
                ...
                case 3: {
                    ...
                    while (rs.next()) {
                        ...
                        TicketDetail td = new TicketDetail(ticketId, matchId, userId, stand, ticketPrice, bookedTickets,
                                totalPaid, payMethod);
                        s.push(td);
                    }
            ...
            while (rs.next()) {
                ...
                while (p > 0) {
                    ...
                    if (otp == fOtp) {
                        ...
                        file.writeBytes("\n1 new Message : ");
                        file.writeBytes("\n   From Your Bank : ");
                        file.writeBytes("\n   Rs. " + total_payments + " Debited From Bank Account Through "
                                + payment_method + ".\n");
                        file.seek(file.length());
                        file.writeBytes("\n" + g.getDateTime());
                        file.writeBytes("\n1 new Message : ");
